...
# implement change of resonance to graphs (detuning)
#convert the joules to eV
#significant figures in the legend
#theta described in the figure, perhaps as an inset in the figure. 
...
def main():
    ...
    ax.add_artist(Circle((0, 0), sphere_radius, fill=False))
    plt.savefig("electric field lines streamplot",
    ...
    plt.colorbar()
    angles = np.linspace(-math.pi/2.5, math.pi/2.5, trajectories)
    xvelocities = np.zeros((trajectories-1,timesteps))
    yvelocities = np.zeros((trajectories-1,timesteps))
    for i in range(trajectories - 1):
        ...
        y = r_trajectory * math.cos(phi_trajectory) * \
            math.sin(theta_trajectory)
        z = r_trajectory * math.sin(phi_trajectory) * \
            math.sin(theta_trajectory)
        ...
        xvel = []
        yvel = []
        electron_positions, graph_position, electron_velocity_position = positions(
            electron_positions, graph_position, incident_field, electron_velocity_position, timestep, x, y, z, timesteps)
        for t in range(timesteps-1):
            xpos.append(electron_positions[t][0])
            ypos.append(electron_positions[t][1])
            xvelocities[i][t] = electron_velocity_position[t][0]
            yvelocities[i][t] = electron_velocity_position[t][1]
        plt.plot(xpos, ypos)
    ...
    plt.pcolormesh(xr, yr, Z)
    plt.colorbar()
    plt.savefig("enhancement" ,
                dpi=300, bbox_inches='tight')
    plt.close()


    #CODE for gettting the kinetic energies or the velocities 
    for i in range(trajectories-1):
        velocity = []
        time_array = []
        m = 0
        for t in range (timesteps-1):
            #velocity.append(math.sqrt((xvelocities[i][t])**2 + (yvelocities[i][t])**2))
            velocity.append(((xvelocities[trajectories -2 -i][t])**2 + (yvelocities[trajectories-2-i][t])**2)*electron_mass*0.5/e)
            time_array.append(2*m)
            m += 1
        plt.plot(time_array,velocity)
    plt.ylabel("Electron energies (eV)")
    plt.xlabel("Time (fs)")
    plt.title("Amplitude 8e5 V/m")
    plt.gca().legend(angles, title_fontsize=30,loc='center left', bbox_to_anchor=(1, 0.5))
    plt.savefig("electron velocities for different angles",
                dpi=300, bbox_inches='tight')
    plt.close()
    #Creating graphs for electron velocity/ kinetic energy and also creating graphs for the enhancmemnt in a line. 
    x_axis = np.linspace(-5*sphere_radius, 5*sphere_radius, nx)
    y_axis = np.linspace(-5*sphere_radius, 5*sphere_radius, nx)
    x_enhancement = []
    y_enhancement = []
    for i in range(nx):
        x_enhancement.append(enhancement(x_axis[i],0))
        y_enhancement.append(enhancement(0,y_axis[i]))
    plt.plot(x_axis,x_enhancement)
    plt.xlabel("x(m)")
    plt.ylabel('enhancement')
    plt.savefig("Electric field enhancement along the x axis",
                dpi=300, bbox_inches='tight')
    plt.close()
    plt.plot(y_axis,y_enhancement)
    plt.xlabel('y(m)')
    plt.ylabel('enhancement')
    plt.savefig("Electric field enhancement along the xyaxis",
                dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for i in range(trajectories - 1):
        phi_trajectory = 0
        theta_trajectory = angles[i]
        phases = theta_trajectory
        temp_field = np.zeros((20, 3))
        time = 0
        for t in range(trajectories):
            Ex = amplitude * math.cos(omega*time + phases) * direction[0]
            Ey = amplitude * math.cos(omega*time + phases) * direction[1]
            Ez = amplitude * math.cos(omega*time + phases) * direction[2]
            temp_field[t][0] = Ex
            temp_field[t][1] = Ey
            temp_field[t][2] = Ez
            time += timestep
        ...
        init_time = 0
        electron_positions, graph_position, electron_velocity_position = positions(
            electron_positions, graph_position, temp_field, electron_velocity_position, timestep, x, y, z, trajectories)
        for t in range(trajectories):
            time.append(init_time)
            dist.append(
            math.sqrt(electron_positions[t][0]**2 + electron_positions[t][1]**2) - radius)
            init_time += timestep
        plt.plot(time, dist)
    ...
    ellipsoid_permittivity = (((-1/L_factor_R_1) + 1)
                              * surrounding_permittivity) +0.0001
    print(L_factor_R_1)
    free_space = e_0
    ...
    if 0 <= theta < math.pi/4 or math.pi*3/4 < theta <= 2*math.pi:
        ...
        y = (a*b)*(math.tan(theta)) / \
                (math.sqrt((b**2) + (a**2) * (math.tan(theta)**2)))
    else:
        x = -(a*b) / (math.sqrt((b**2) + (a**2) * (math.tan(theta)**2)))
        y = -(a*b)*(math.tan(theta)) / \
                (math.sqrt((b**2) + (a**2) * (math.tan(theta)**2)))
    ...
    timestep_graph_ellipse(electron_position_ellipse,
                           timesteps, electron_velocity_ellipse, timestep)
    ...
    ex, ey = E_ellipsoid2(x=X, y=Y, f1=f1, L_factor_R_1=L_factor_R_1,
                          ellipsoid_permittivity=ellipsoid_permittivity)
    plot_Ex = ex
    plot_Ey = ey
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ...
    ax.streamplot(xr, yr, plot_Ex, plot_Ey, color=color, linewidth=0.2,cmap=plt.cm.inferno,
                  density=2, arrowstyle='->', arrowsize=0.5)
    plt.savefig("electric field lines streamplot ellipsoid",
                dpi=300, bbox_inches='tight')
    plt.close()
    nx, ny = 1000, 1000  # level of discretisation
    xr = np.linspace(-5*sphere_radius, 5*sphere_radius, nx)
    ...
    Z = enhancement_ellipsoid2(
        x=X, y=Y, f1=f1, L_factor_R_1=L_factor_R_1, ellipsoid_permittivity=ellipsoid_permittivity)
    plt.pcolormesh(xr, yr, Z)
    plt.colorbar()
    xvelocities = np.zeros((trajectories,timesteps))
    yvelocities = np.zeros((trajectories,timesteps))
    for i in range(trajectories - 1):
        phi_trajectory = 0
        theta_trajectory = angles[i]
        phi_trajectory = phi_trajectory % math.pi
        ...
        graph_position = []
        electron_velocity_position = np.zeros((timesteps, 3))
        if 0 <= theta_trajectory < math.pi/4 or math.pi*3/4 < theta_trajectory <= 2*math.pi:
            x = (a*b) / (math.sqrt((b**2) + (a**2) * (math.tan(theta_trajectory)**2)))
            y = (a*b)*(math.tan(theta_trajectory)) / \
                (math.sqrt((b**2) + (a**2) * (math.tan(theta_trajectory)**2)))
        else:
            x = -(a*b) / (math.sqrt((b**2) + (a**2) * (math.tan(theta_trajectory)**2)))
            y = -(a*b)*(math.tan(theta_trajectory)) / \
                (math.sqrt((b**2) + (a**2) * (math.tan(theta_trajectory)**2)))
    #z = r * math.sin(phi) * math.sin(theta)
        z = 0
        #print(theta_trajectory)
        #print(math.tan(theta_trajectory))
        #print(x,y)
        electron_positions[0][0] = x + 0.0000000001
        electron_positions[0][1] = y
        electron_positions[0][2] = z
        xpos = []
        ypos = []
        xvel = []
        yvel = []
        electron_positions, graph_position, electron_velocity_position = positions_ellipsoid(
            electron_positions, graph_position, incident_field, electron_velocity_position, timestep, L_factor_R_1, ellipsoid_permittivity, free_space)
        for t in range(timesteps-1):
            xpos.append(electron_positions[t][0])
            ypos.append(electron_positions[t][1])
            xvelocities[i][t] = electron_velocity_position[t][0]
    ...
    plt.pcolormesh(xr, yr, Z)
    plt.colorbar()
    plt.savefig('enhancement ellipsoid',
                dpi=300, bbox_inches='tight')
    plt.close()
    nx, ny = 1000, 1000  # level of discretisation
    xr = np.linspace(-5*sphere_radius, 5*sphere_radius, nx)
    ...
    plt.xlim(-3*sphere_radius, 3*sphere_radius)
    plt.ylim(-3*sphere_radius, 3*sphere_radius)
    enhancement_ellipsoid2 = np.vectorize(enhancement_ellipsoid_x)
    Z = enhancement_ellipsoid2(
        x=X, y=Y, f1=f1, L_factor_R_1=L_factor_R_1, ellipsoid_permittivity=ellipsoid_permittivity)
    plt.pcolormesh(xr, yr, Z)
    plt.colorbar()
    plt.savefig('enhancement x ellipsoid',
                dpi=300, bbox_inches='tight')
    plt.close()
    nx, ny = 1000, 1000  # level of discretisation
    xr = np.linspace(-5*sphere_radius, 5*sphere_radius, nx)
    ...
    plt.xlim(-3*sphere_radius, 3*sphere_radius)
    plt.ylim(-3*sphere_radius, 3*sphere_radius)
    enhancement_ellipsoid2 = np.vectorize(enhancement_ellipsoid_Y)
    Z = enhancement_ellipsoid2(
        x=X, y=Y, f1=f1, L_factor_R_1=L_factor_R_1, ellipsoid_permittivity=ellipsoid_permittivity)
    plt.pcolormesh(xr, yr, Z)
    plt.colorbar()
    plt.savefig('enhancment y ellipsoid',
                dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for i in range(trajectories -1):
        ...
        for t in range (timesteps-1):
            #velocity.append(math.sqrt((xvelocities[i][t])**2 + (yvelocities[i][t])**2))
            accel = ((xvelocities[i][t])**2 + (yvelocities[i][t])**2)*electron_mass*0.5/e
            #print(accel)
            velocity.append(accel)
            time_array.append(2*m)
            m += 1
        plt.plot(time_array,velocity)
    plt.gca().legend(angles, title_fontsize=30,loc='center left', bbox_to_anchor=(1, 0.5))
    ...
    plt.xlabel("Time (fs)")
    plt.savefig("electron ellipsoid velocities for different angles",
                dpi=300, bbox_inches='tight')
    plt.close()
    x_axis = np.linspace(-5*sphere_radius, 5*sphere_radius, nx)
    y_axis = np.linspace(-5*sphere_radius, 5*sphere_radius, nx)
    ...
    y_enhancement = []
    for i in range(nx):
        x_enhancement.append(enhancement_ellipsoid_x(x_axis[i],0,Ex,L_factor_R_1,ellipsoid_permittivity))
        y_enhancement.append(enhancement_ellipsoid_Y(0,y_axis[i],Ex,L_factor_R_1,ellipsoid_permittivity))
    plt.plot(x_axis,x_enhancement)
    plt.xlabel("x(m)")
    plt.ylabel('enhancement')
    plt.savefig("Electric field enhancement along the x axis ellipsoid",
                dpi=300, bbox_inches='tight')
    plt.close()
    plt.plot(y_axis,y_enhancement)
    plt.xlabel('y(m)')
    plt.ylabel('enhancement')
    plt.savefig("Electric field enhancement along the yaxis ellipsoid",
                dpi=300, bbox_inches='tight')
    plt.close()
    print("done")
...
def timestep_graph(electron_position, timestep, graph_field, electron_velocity):
    ...
    for t in range(timesteps-1):
        ...
        xkin.append((electron_velocity[t][0]**2)*0.5* electron_mass)
        ykin.append((electron_velocity[t][1]**2)*0.5* electron_mass)
        zkin.append((electron_velocity[t][2]**2)*0.5* electron_mass)
    plt.figure(0)
    ...
    plt.close()
    plt.plot(timestep_for_graph, xkin)
    plt.savefig("xKin of electron.png", dpi=300, bbox_inches='tight')
    plt.close()
    plt.plot(timestep_for_graph, ykin)
    plt.savefig("yKin of electron.png", dpi=300, bbox_inches='tight')
    plt.close()
    plt.plot(timestep_for_graph, zkin)
    plt.savefig("zKin of electron.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def timestep_graph_ellipse(electron_position, timesteps, electron_velocity, timestep):
    ...
    for t in range(timesteps-1):
        ...
        zvel.append(electron_velocity[t][2])
    plt.plot(timestep_for_graph, xpos)
    plt.savefig("Xposition of electron ellipse.png",
                dpi=300, bbox_inches='tight')
    plt.close()
    plt.plot(timestep_for_graph, ypos)
    plt.savefig("Yposition of electron ellipse.png",
                dpi=300, bbox_inches='tight')
    plt.close()
    plt.plot(timestep_for_graph, zpos)
    plt.savefig("Zposition of electron ellipse.png",
                dpi=300, bbox_inches='tight')
    plt.close()
    plt.plot(xpos, ypos)
    plt.savefig("X-Y trajectory of electron ellipse.png",
                dpi=300, bbox_inches='tight')
    plt.close()
...
def enhancement(x, y):
    ...
    if inside_check(x, y, z) == True:
        Ex = (3*surrounding_permittivity
              / (sphere_permittivity + 2 * surrounding_permittivity))
        Ey = 0
        return math.sqrt((Ex**2) + (Ey**2))/10000
...
def E_ellipsoid(x, y, f1, L_factor_R_1, ellipsoid_permittivity):
    prefactor = (R_1*R_2*R_3/2) * ((surrounding_permittivity
                                    - ellipsoid_permittivity)/ellipsoid_permittivity)
    V = (R_1*R_2*R_3) * 1e-24
    z = 0
    r = math.sqrt(x**2 + y**2 + z**2)
    if ellipsoid_inside_check(x, y, z) == False:
        E_x = -f1/(1 + ((ellipsoid_permittivity-surrounding_permittivity)*L_factor_R_1/surrounding_permittivity))
        E_y = 0
    else:
        E_x = -f1 * (-1 + surrounding_permittivity*(ellipsoid_permittivity - surrounding_permittivity) * ((surrounding_permittivity))
                    / (surrounding_permittivity + L_factor_R_1 * (ellipsoid_permittivity - surrounding_permittivity)) * (V/r**5) * (2 * ((x/R_1)**2) - ((y/R_2)**2) - ((z/R_3)**2)))
        E_y = -f1 * (surrounding_permittivity * (ellipsoid_permittivity - surrounding_permittivity) * ((surrounding_permittivity))
                    / (surrounding_permittivity + L_factor_R_1 * (ellipsoid_permittivity - surrounding_permittivity)) * (V/r**5) * (3*x*y/(R_1*R_2)))
    return E_x, E_y
...
def E_ellipsoid_outside(x, y, z, L_factor_R_1, current_field, ellipsoid_permittivity, free_space):
    ...
    V = (R_1*R_2*R_3)*1e-24
    r = math.sqrt(x**2 + y**2 + z**2)
    P_x = free_space * (sphere_permittivity - surrounding_permittivity) * ((surrounding_permittivity)
                                                                           * current_field[0])/(surrounding_permittivity + L_factor_R_1 * (sphere_permittivity - surrounding_permittivity))
    if ellipsoid_inside_check(x, y, z) == False:
        E_x = (current_field[0] - L_factor_R_1
               * P_x/(e_0 * surrounding_permittivity))
        E_y = 0
        E_z = 0
    else:
        E_x = current_field[0] * (1 + (ellipsoid_permittivity - surrounding_permittivity) * ((surrounding_permittivity))/(
            surrounding_permittivity + L_factor_R_1 * (ellipsoid_permittivity - surrounding_permittivity)) * (V/r**5) * (2 * ((x/R_1)**2) - ((y/R_2)**2) - ((z/R_3)**2)))
        E_y = current_field[0] * (ellipsoid_permittivity - surrounding_permittivity) * ((surrounding_permittivity))/(
            surrounding_permittivity + L_factor_R_1 * (ellipsoid_permittivity - surrounding_permittivity)) * (V/r**5) * (3*x*y/(R_1*R_2))
        E_z = current_field[0] * (ellipsoid_permittivity - surrounding_permittivity) * ((surrounding_permittivity))/(
            surrounding_permittivity + L_factor_R_1 * (ellipsoid_permittivity - surrounding_permittivity)) * (V/r**5) * (3*z*y/(R_2*R_3))
    return [-E_x, -E_y, -E_z]
...
def enhancement_ellipsoid(x, y, f1, L_factor_R_1, ellipsoid_permittivity):
    ...
    z = 0
    r = math.sqrt(x**2 + y**2 + z**2)
    P_x = f1 / (L_factor_R_1 + 1
                / (ellipsoid_permittivity + surrounding_permittivity))
    if ellipsoid_inside_check(x, y, z) == False:
        ...
        E_y = f1 * (surrounding_permittivity * (ellipsoid_permittivity - surrounding_permittivity) * ((surrounding_permittivity))
                    / (surrounding_permittivity + L_factor_R_1 * (ellipsoid_permittivity - surrounding_permittivity)) * (V/r**5) * (3*x*y/(R_1*R_2)))
    return math.sqrt(E_x**2 + E_y**2)/1e5

def enhancement_ellipsoid_x(x, y, f1, L_factor_R_1, ellipsoid_permittivity):
    prefactor = (R_1*R_2*R_3/2) * ((surrounding_permittivity
                                    - ellipsoid_permittivity)/ellipsoid_permittivity)
    V = (R_1*R_2*R_3) * 1e-24
    z = 0
...
def enhancement_ellipsoid_Y(x, y, f1, L_factor_R_1, ellipsoid_permittivity):
    prefactor = (R_1*R_2*R_3/2) * ((surrounding_permittivity
                                    - ellipsoid_permittivity)/ellipsoid_permittivity)
    V = (R_1*R_2*R_3) * 1e-24
    z = 0
